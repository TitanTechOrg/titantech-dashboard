...
const getTitanHealthPercentage = (
  titan: TitanSequence | undefined,
  partType: 'Body' | 'Armor'
): number => {
  if (!titan) return 0;

  const markedParts = titan.parts.filter(
    (part) => part.target && part.name.includes(partType)
  );
  const totalHealth = markedParts.reduce((prev, curr) => prev + curr.health, 0);
  const totalRemainingHealth = markedParts.reduce(
    (prev, curr) => prev + curr.current_health,
    0
  );

  const remainingTitanBodyHealth =
    titan.health - totalHealth + totalRemainingHealth;

  let result =
    partType === 'Armor'
      ? percentage(totalHealth, totalRemainingHealth)
      : percentage(titan.health, remainingTitanBodyHealth);
...
const getTitanHealth = (
  ...
  const result =
    partType === 'Armor' ? totalRemainingHealth : remainingTitanBodyHealth;
    ...
    <Card className="h-full w-full min-w-72 p-2 dark:bg-neutral-800">
      ...
      <CardBody className="flex flex-col gap-4">
        ...
        <Skeleton isLoaded={!!titan} className="min-w-full rounded-md">
          {titan && (
            <TitanHealthBars
              titanArmourHealthPercentage={getTitanHealthPercentage(
                titan,
                'Armor'
              )}
              titanBodyHealthPercentage={getTitanHealthPercentage(
                titan,
                'Body'
              )}
          ...
          <div className="flex flex-row">
            <Skeleton isLoaded={!!titan} className="min-w-full rounded-md">
              <h3 className="text-medium font-medium">
                {hasActiveConditionals() ? 'Active conditional cards' : null}
              </h3>
            </Skeleton>
          </div>
          {hasActiveConditionals() ? (
