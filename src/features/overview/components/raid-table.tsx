...
import { abbreviateNumber, formatTime } from '@/utils';
import {
  Button,
  Chip,
  ChipProps,
...
export function RaidTable({ raidId, raid }: RaidTableProps) {
            ...
            <div className="flex flex-row items-center justify-end">
              {typeof text === 'number' ? (
                ...
                <p className="text-bold">
                  {text[0]}
                  <span className="inline-block w-4 pl-1 text-left">
                    {text[1]}
                  </span>
                </p>
              )}
        ...
        case 'attack_count': {
          const currentCycle =
            selectedStatusValue === 'all'
              ? (raidCycles?.cycles?.length ?? 1)
              : 1;
          return (
            <div className="flex flex-row items-center justify-center">
              <Chip
                color={getAttacksStatusColour(
                  currentCycle,
                  player.attack_count,
                  raid.attacksPerTier
                )}
              ...
              >
                {cellValue}/
                {countTotalAttacks(currentCycle, raid.attacksPerTier)}
              </Chip>
            </div>
          );
        }
        case 'duration':
          return (
            <span className="flex items-start justify-center">
              {formatTime(Number(cellValue))}
            </span>
          );

        default:
          return (
            <span className="flex items-start justify-start">{cellValue}</span>
          );
      }
    },
    [selectedStatusValue, raidCycles?.cycles?.length, raid.attacksPerTier]
  );
          ...
          <div className="flex items-center justify-between">
            {overviewPlayers && overviewPlayers?.players_data.length > 0 && (
              <span className="text-small text-default-400">
                Total {overviewPlayers.players_data.length} players
              </span>
            )}
          </div>
            ...
            <Dropdown>
              <DropdownTrigger className="hidden sm:flex">
                <Button
                  ...
                  variant="flat"
                  isDisabled={disableCycleDropdown}
                >
                  Rounds
                </Button>
              </DropdownTrigger>
            ...
            <Dropdown>
              ...
              >
                {columns.map((column) => (
                  <DropdownItem
                    key={column.uid}
                    isReadOnly={column.uid === 'player_name'}
                  >
                    {column.name}
                  </DropdownItem>
                ))}
  ...
  return (
    ...
    >
      ...
      <TableBody
        emptyContent={
          isLoading ? <Spinner label="Loading..." /> : <p>No players found</p>
        }
        items={sortedItems}
        isLoading={isLoading}
      >
