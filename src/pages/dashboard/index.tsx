...
import { CardBonusData, PercentageCardsType, RaidCycle, RaidDamageInfo, RaidInfo, useRaidCycles, useRaidList } from '@/features/raid-info';
...
export default function Dashboard() {
    ...
    const calculateMoraleBonuses = (raidCycles: RaidCycle[]) =>
        raidCycles.map(({ morale, team_tactics }) => ((morale + team_tactics) * 100).toFixed(2));

    const calculateMirrorForceBonuses = (raidCycles: RaidCycle[]) => raidCycles.map(({ mirror_force }) => (mirror_force * 100).toFixed(0));
