  ...
  return (
    <div className="flex flex-row flex-wrap gap-x-1">
      {items.map((value, index) => (
        <CardPreviousValue
          key={`${index}_raid_damage_stats_${value}`}
          index={index}
          value={value}
...
export function RaidDamageInfo() {
  ...
  const data = useMemo(() => {
    ...
    const getAverageClanDamage = (raidCycles: RaidCycle[]) =>
      raidCycles.map(({ average_damage }) => Math.round(average_damage));
    ...
    return mapDamageData(
      getAverageClanDamage(raidCycles.cycles),
      getOverallClanDamage(raidCycles.cycles)
    );
  }, [raidCycles]);
    ...
    <Card className="h-full w-full px-2 dark:bg-neutral-800">
      <CardHeader className="flex h-[70px] justify-between text-lg font-medium">
        ...
        <span className="text-xl font-semibold">
          {formatter().format(overallRaidDamage)}
        </span>
      </CardHeader>
      <Divider />
      <CardBody className="flex h-72 flex-col gap-2">
        <CardValues
          items={data.map(({ average }) => formatter().format(average))}
        />
