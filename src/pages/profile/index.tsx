...
export default function PlayerProfile() {
  ...
  const { data: overviewPlayers, isLoading: isLoadingOverviewPlayers } =
    useOverviewPlayers(raidId);
  const { selectedValues: selectedPlayerId } = useSelectStore();
  const {
    data: playerProfile,
    isLoading: isLoadingProfile,
    isError: isErrorLoadingPlayer,
  } = usePlayerProfile(selectedPlayerId['player-selector']);
    ...
    <PageContainer>
      <div className="grid grid-cols-1 gap-6 text-left sm:grid-cols-2">
        <div className="col-span-2 row-span-1 row-start-1">
          <div className="mb-2 flex flex-col items-start justify-center">
            ...
            <p className="text-xs italic">
              Players are fetched based on previous raid
            </p>
          </div>
          {overviewPlayers ? (
            <PlayerSelect
              overviewPlayers={overviewPlayers}
              isLoading={isLoadingOverviewPlayers}
            />
          ) : (
        ...
        {isLoadingProfile ? (
          ...
          <div className="col-span-2 mx-auto justify-center">
            Player data could not be fetched. Is the player currently in the
            clan?
          </div>
        ) : (
          playerProfile && (
