...
export function NecrobearResearchTree() {
  ...
  return (
    <div className="flex flex-col items-center p-4">
      <div className="bg-default-100 mb-8 w-full max-w-md rounded-lg p-4">
        <details>
          <summary className="cursor-pointer font-medium">View Bonuses</summary>
          <ul className="flex flex-col gap-2 pl-4">
            {bonuses.map((bonus, idx) => (
              <li
                key={`${bonus.type}_${idx}`}
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            <Button
              ...
              color="primary"
              aria-label="Reset all nodes to level 0"
            >
              Reset All
            </Button>
            <Button
              onPress={handleMaxAll}
              color="primary"
              aria-label="Maximize all nodes to their total level"
            >
              Max All
            </Button>
          </div>
          ...
          <div key={`${key}_${outerIdx}`} className="mb-4 flex space-x-4">
            {nodeGroup.map((_, innerIdx) => {
              ...
              return (
                <div
                  key={currentNode.id}
                  className="flex-grow rounded-lg border p-4"
                >
                  <div className="flex flex-col items-center gap-2 text-left">
                    <h3 className="text-lg font-bold">{currentNode.type}</h3>
                    <p>Bonus per Level: {currentNode.levels.bonusPer}%</p>
                    <p>
                      Current Level: {currentNode.levels.current} /{' '}
                      {currentNode.levels.total}
                    </p>
                    <p>
                      Total Bonus:{' '}
                      {currentNode.levels.current * currentNode.levels.bonusPer}
                      %
                    </p>
                    ...
                    <ButtonGroup>
                      ...
                      <Button
                        ...
                        isIconOnly
                        disabled={
                          currentNode.levels.current >= currentNode.levels.total
                        }
