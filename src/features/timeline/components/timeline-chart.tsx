...
const pickColour = (index: number) => {
    ...
    switch (index) {
        case 0:
            return ['hsl(212.14 92.45% 58.43% / 1)', `hsl(212.14 92.45% 58.43% / ${opacity})`];
        case 1:
            return ['hsl(270 59.26% 57.65% / 1)', `hsl(270 59.26% 57.65% / ${opacity})`];
        case 2:
            return ['hsl(146.01 62.45% 55.1% / 1)', `hsl(146.01 62.45% 55.1% / ${opacity})`];
        case 3:
            return ['hsl(37.01 91.26% 64.12% / 1)', `hsl(37.01 91.26% 64.12% / ${opacity})`];
        case 4:
...
const generateCycleData = (cycle: Timeline['cycles'][0], index: number) => {
    const dataMap = new Map(cycle.hours.map(({ hour, attacks }) => [hour, attacks]));
    const dataArray = Array.from({ length: 24 }, (_, hour) => dataMap.get(hour) || null);
...
export function TimelineChart() {
    ...
    const chartData = useMemo(
        () => ({
            labels,
            datasets: data?.cycles.map((cycle, idx) => generateCycleData(cycle, idx)) || [],
        }),
        [data, labels]
    );
    ...
    return (
        <Card className="h-full w-full px-2 dark:bg-neutral-800">
            <CardHeader className="flex h-[70px] items-center justify-between text-lg font-medium">Attacks timeline</CardHeader>
            <Divider />
