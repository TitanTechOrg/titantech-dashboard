...
export function AlchemyCalculator() {
      ...
      z.object(
        Object.fromEntries(
          ingredients
            ...
            .map((ingredient) => [ingredient.name, ingredient.fieldType])
        )
      )
    ),
  });
  ...
  const onSubmit = handleSubmit((formData: FieldValues) => {
    ...
    const available_ingredients = Object.fromEntries(
      ingredients
        .filter((ingredient) => ingredient.active)
        .map((ingredient) => [ingredient.name, formData[ingredient.name] || 0])
    ...
    const payload: IngredientsData = {
      ...
      base_ingredients: ingredients
        .filter((ingredient) => ingredient.base)
