...
const findSkeletalSmashTarget = (data: TitanSequenceParts[]) => {
  ...
  let ssTargetPart = Object.keys(TitanPartMapPlayerPerspective).find(
    (key) =>
      TitanPartMapPlayerPerspective[key as RaidTitanPartPlayerPerspective] ===
      foundTarget.name
  );
...
function SkeletalSmashCard({ text }: SkeletalSmashCardProps) {
  return (
    <div className="bg-default mx-auto mt-4 flex max-w-fit flex-row items-center justify-center gap-2 rounded-lg p-2">
      <div className="min-w-fit">
        <Image
          src={SkeletalSmash}
          className="flex h-8 w-8 rounded-sm object-cover"
          alt="Skeletal Smash icon"
        />
      </div>
...
export function RaidTitanData({
  titan,
  damagedParts,
  showHealthbars,
}: RaidTitanDataProps) {
  const ssTarget = titan
    ? findSkeletalSmashTarget(getTitanCursedParts(titan).parts)
    : null;
  return (
    <div className="flex flex-col flex-wrap">
      {titan && (
        <TitanPartTableData
          ...
          titanData={getTitanCursedParts(titan)}
          showHealthbars={showHealthbars}
        />
      )}

      {ssTarget && showHealthbars ? (
        <SkeletalSmashCard text={ssTarget} />
      ) : null}
    </div>
  );
}
