...
export function TitansSequence() {
  ...
  const [selectedOption, setSelectedOption] = useState(
    new Set([currentTitan?.id!])
  );

  const isDefeatedTitan = (otherTitanSequenceIndex: number) =>
    !!(currentTitan && currentTitan?.sequence_index > otherTitanSequenceIndex);
    ...
    <Card className="h-full p-2 dark:bg-neutral-800">
      <CardHeader className="flex h-[70px] flex-row items-center justify-between">
        ...
        <div className="min-w-fit">
          <Skeleton isLoaded={!!selectedTitan} className="rounded-md">
            <Image
              src={TitanSequenceImage}
              className="flex h-8 w-8 object-cover"
              radius="none"
            />
          </Skeleton>
        </div>
      ...
      <CardBody className="flex flex-col gap-4">
                ...
                <Dropdown placement="top">
                      ...
                      startContent={
                        <div className="relative flex w-full flex-row items-center justify-start gap-2">
                          ...
                          <Image
                            ...
                            src={
                              TitanImageMapping[
                                selectedTitan.name as TitanImageMappingType
                              ]
                            }
                          />
                          <span>
                            {selectedTitan.name} &nbsp;
                            {selectedTitan.sequence_index! + 1} /{' '}
                            {titans.length}
                          </span>
                        </div>
                      }
                  ...
                  <DropdownMenu
                    ...
                    onSelectionChange={(keys: Selection) =>
                      selectTitan(Array.from(keys)[0].toString())
                    }
                        ...
                        startContent={
                          <div className="relative h-auto min-w-fit">
                            ...
                            <Image
                              ...
                              src={
                                TitanImageMapping[
                                  titan.name as TitanImageMappingType
                                ]
                              }
                            />
                          </div>
        ...
        <div className="flex flex-row items-center justify-center">
          <Skeleton isLoaded={!!selectedTitan} className="rounded-md">
            {selectedTitan && (
              <RaidTitanData titan={selectedTitan} showHealthbars={true} />
            )}
          </Skeleton>
        </div>
      </CardBody>
