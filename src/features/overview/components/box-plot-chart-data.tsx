...
type ParsedTooltipDatapointTypes = {
  max: number;
  mean: number;
  median: number;
  min: number;
...
type MappedTooltipDatapointTypes = {
  q0: string;
  q1: string;
  q2: string;
  q3: string;
...
const getOrCreateTooltip = (chart: any) => {
  ...
  if (!tooltipEl) {
    ...
    tooltipEl.className =
      'rounded-sm text-white opacity-100 pointer-events-none absolute w-44 bg-black/50';
...
const constructElement = (title: string, value: string) => {
  ...
  paragraphElement.className =
    'flex flex-row justify-between gap-8 text-xs font-semibold';
...
const externalTooltipHandler = (context: {
  ...
  if (tooltip.body) {
    ...
    const dataPoints: MappedTooltipDatapointTypes[] = bodyLines.map(
      ({ max, mean, median, min, q1, q3 }: ParsedTooltipDatapointTypes) => {
        return {
    ...
    dataPoints.forEach(
      ({ q4, q3, q2, q1, q0, range, average }: MappedTooltipDatapointTypes) => {
  ...
  const options: ChartOptions<'boxplot'> = {
    ...
    scales: {
      ...
      y: {
        ...
        ticks: {
          callback: (label) =>
            formatter(0, 0).format(Math.trunc(Number(label))),
        },
        grid: {
          color: darkModeStorage
            ? CHART_GRID_COLOUR.dark
    ...
    <div className="relative h-[512px] w-[1024px] sm:h-72 sm:w-dvw sm:max-w-full">
      <Suspense fallback={<Spinner label="Loading chart..." />}>
        <Chart
          type="boxplot"
          options={options}
          data={mapBoxPlotPlayerData(playersData)}
        />
      </Suspense>
    </div>
  );
