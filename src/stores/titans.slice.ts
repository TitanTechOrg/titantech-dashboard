...
export const createTitanSlice: StateCreator<
  TitanSliceState,
  [],
  [],
  TitanSliceState
> = (set, get) => ({
  ...
  setCurrentTitan: (currentTitan: TitanSequence) =>
    set(() => ({ currentTitan })),
  isInsanityVoid: () => {
    ...
    const markedArmouredParts = markedParts.filter((part) =>
      part.name.includes('Armor')
    );
    const markedBodyParts = markedParts.filter((part) =>
      part.name.includes('Body')
    );

    const isAllArmouredPartsBroken = markedArmouredParts.every(
      (part) => part.current_health <= 0
    );

    const brokenBodyParts = markedBodyParts.filter(
      (part) => part.current_health <= 0
    );
    const isMoreThanHalfPartsBroken =
      brokenBodyParts.length <= markedBodyParts.length / 2;
  ...
  isSkeletalSmash: () => {
    ...
    const hasAtLeastOneSkeletonPart = markedBodyParts.some(
      (part) => part.current_health <= 0
    );
    const hasAtLeastOneRemainingArmouredPart = markedArmouredParts.some(
      (part) => part.current_health > 0
    );
  ...
  isDecayingStrike: () => {
    ...
    const hasAtLeastOnePartUnderThreshold = markedParts.some((part) => {
      const remainingHealthPercentage = percentage(
        part.health,
        part.current_health
      );
      return (
        part.current_health > 0 &&
        remainingHealthPercentage <= decayingStrikeThreshold
      );
    });
  ...
  isVictoryMarch: () => {
    ...
    const isAllButOneBodyPartsBroken =
      brokenBodyParts.length === markedBodyParts.length - 1;
