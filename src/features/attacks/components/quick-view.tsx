...
const isDamagedOffstratPart = (
  ...
  const foundDamagedArmourPart = damagedParts.find(
    (part) => part.name === partArmourName
  );
  const foundDamagedBodyPart = damagedParts.find(
    (part) => part.name === partArmourName
  );

  const hasDamagedOffstratPart =
    !!foundDamagedArmourPart || !!foundDamagedBodyPart;
  ...
  const hasOffstratDamageOverThreshold =
    armourDamage > offstratDamageThreshold ||
    bodyDamage > offstratDamageThreshold;

  return hasOffstratDamageOverThreshold;
};

const getGradientColour = (
  armourPart: PartAndColour,
  bodyPart: PartAndColour
) => {
  ...
  return (
    <div className={`${classes}`}>
      ...
      <div className="flex flex-row items-center justify-center gap-0">
        ...
        >
          {isDamagedOffstratPart(
            offstratDamageThreshold,
            'ArmorArmUpperRight',
            'BodyArmUpperRight',
            titan?.parts,
            data
            ...
            <div className="h-full min-h-6 w-full min-w-6">
              <Image
                width="100%"
                height="100%"
                className="absolute top-1/2 left-0 scale-[33%]"
                src={OffStratDamage}
      ...
      <div className="flex flex-row items-center justify-center gap-0">
        <div
          className={`relative h-6 w-6 border-1 border-t-0 border-r-0 border-gray-400 ${getGradientColour(
